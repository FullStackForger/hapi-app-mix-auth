...
internals.implementation = function (server, options) {
	...
	scheme.authenticate = function (request, reply) {
        ...
        internals.settings.validateFunc.call(request, method, authObject.data, function (err, isValid, credentials) {
...
internals.getToken = function (tokenHash) {
	...
	if (!authObject.data.provider) {
		authObject.error = Boom.badRequest('HTTP authentication header missing provider', internals.LABEL);
		return authObject;
	}

	if (!authObject.data.token) {
		authObject.error = Boom.badRequest('HTTP authentication header missing token', internals.LABEL);
		return authObject;
	}
